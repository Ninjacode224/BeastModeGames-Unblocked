...
const appClicked: any = function(event: any) {
  if (!event.nativeEvent.path) var el = event.nativeEvent.target.getAttribute('Beast Mode Games-launch-url')?event.nativeEvent.target:event.nativeEvent.target.parentNode;
  else var el = (event.nativeEvent.path||[]).find((e:any)=>e.classList&&e.classList.contains(styles['app']));

  var app = el.getAttribute('Beast Mode Games-launch-url');
  ...
  return (
    <div className={styles.main}>
      <Head>
        ...
        <title>Beast Mode Games - Apps</title>
            ...
            <div id={styles['launch-over']}>
              {
                apps.map((app: any) => {
                  if (app.proxy==false) {
                    return (
                      <div onClick={noAppClicked} key={app.id} className={styles.app} Beast Mode Games-launch-url={app.url}>
                  ...
                  return (
                    <div onClick={appClicked} key={app.id} className={styles.app} Beast Mode Games-launch-url={app.url}>
                      <img src={app.icon} alt='' />
                      <h3>{app.name}</h3>
                    </div>
                  )
            ...
            <div id={styles['games-over']}>
              {
                games.map((game: any) => {
                  return (
                    <div key={game.id} onClick={gameClicked} Beast Mode Games-launch-url={game.id} className={styles.app}>
